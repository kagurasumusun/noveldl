...
## 追加修正その12 — 表紙の統一・棚の再設計・YAMLフォーム編集・サイト一覧の実態化

| 指摘 | 対応 |
|---|---|
| 表紙の「2画像の縦横結合」 | **横広1画像のみ**の `WideCover` に統一(実カバー画像があればそれを、無ければ生成紙面)。詳細の合成表示(横バナー+縦表紙の重ね)を撤去 |
| 本棚の表示(誤読の訂正) | 横並び/グリッド/リスト切替を撤去し、**横広の表紙を縦1列**の本棚に再設計(表紙+取得進捗) |
| YAML編集を「全文書かせる」方式に | **フォーム式サイト編集**(基本/一覧抽出/本文抽出/アクセスの各項目にヒント+例)。新規は雛形を自動投入=全文を書く必要なし。上級者向けに全文 YAML 併設。クッキー/コマンドの個別UIは無し(アプリ内部の仕事) |
| R-18表示が実態と違う | **サイトの実態**を優先して表示(カクヨム/青空/なろう本体=全年齢、ハーメルン/アルファポリス/暁/ノベルアップ=R-18あり、なろうR-18=R-18専)。未知サイトのみプリセットの年齢ゲート設定に従う |
| サイト一覧が見ずらい | **分類表示**(大手 / 創作・投稿 / レーベル・文芸 / その他)+ 1行1サイト(モノグラム+名前+ドメイン+年齢区分チップ+編集導線) |

## 追加修正その13 — YMLインポート

- 対応サイトに「YML読込」を追加:`.yml` / `.yaml` / テキストのファイルを選択 → 編集フォームに展開 → 内容確認して保存(読み込むだけで確定しない安全な流れ)。
- インポート由来の未知キーも保存時に保持(既存 YAML を書き損ねない直列化)。
- 保存後に一覧へ自動反映。

## 追加修正その14 — 目次の複数ページ・読書画面の独立化・バーの整理・挿絵・塩梅調整

| 指摘 | 対応 |
|---|---|
| 複数ページの目次で1ページしか取れない | C:「次へ/rel=next」リンクの**汎用ページ送り検出**を追加(個別ルール無しでもページ分割目次を全部巡回、上限400頁)。テスト追加で122/122 |
| 左→右スワイプで詳細へ戻る / 左上の戻るボタン | 読書を**独立ページ(fullScreenCover)化** — ナビの戻るも端スワイプ戻りも無し。スワイプは送り専用 |
| バーが出たり隠れたり | **中央タップでのみ出没**(タイマー無し・安定)。左右タップ=ページ送り |
| バーのボタン構成 | 上:一覧・タイトル・栞・共有・設定 / 下:前話・前頁・進捗・次頁・次話(小さめアイコン+短い見出しで過不足なく) |
| 中央の四角に入っている感じ | 額縁を廃し**全面レイアウト**(幅制限なし・余白はマージン設定のみ) |
| 挿絵・画像 | `<img>` を実画像で表示(灰色の箱=プレースホルダ→非同期に差し替え・キャッシュあり) |
| 左スワイプで前ページへ / 端で話の自動遷移 | 左→右=前の頁、右→左=次の頁。最初の頁から前へ=前の話、最後の頁から次へ=次の話へ自動遷移 |
| サイト一覧タップで戻る | 編集画面の**入れ子 NavigationStack を削除**(これが即戻りの原因) |
| 塩梅の見直し | バー半透明0.94・ボタン38-40pt・キャプション9pt・ヒントは初回のみ。読書アプリ特有の静けさを優先(汎用UIの賑やかさは採らない) |

## 追加修正その15 — UI/機能の階層整理・送りの一元化・指示の総点検

**階層(UI・機能の置き場所)**: 標準 TabView を廃し、独自の帯タブで画面階層を明示。
本棚 → 作品詳細 → 読書(独立) / さがす / 取得 / 設定 → 対応サイト → 編集。
リーダー操作は読書メニューへ、アプリ操作は設定へ、作品操作は詳細へ一本化。

**送りの一本化**: 縦スクロールと横スワイプの同時有効を解消 — 送りは横スワイプ/左右タップのみ(縦は無効)。
話の端では自動的に次/前の話へ遷移(一本化により確実に発火)。

**指示の総点検(達成状況)**:
| 指示 | 状態 |
|---|---|
| 読書専用メニュー(アプリ設定と分離) | 達成 |
| メニュー/バーは中央タップで出没(常時表示×) | 達成 |
| 戻るボタン常時表示の廃止・独立ページ化 | 達成 |
| バーのボタン(設定/次話/前話/次頁/前頁/共有/ブックマーク/一覧) | 達成 |
| 表紙は横広1画像(2画像合成の撤去) | 達成 |
| ページめくり演出(紙捲り/スライド/フェード/なし)+度合い調整 | 達成 |
| リーダー全面レイアウト(中央の箱の撤去) | 達成 |
| 挿絵・画像の表示 | 達成 |
| 左スワイプで前ページ/端で話の自動遷移 | 達成 |
| 縦スクロールと横スワイプの同時有効をやめる | 達成 |
| 複数ページ目次の取得 | 達成(C:汎用ページ送り検出) |
| YAML編集をフォーム化(全文書かせない)+YML読込 | 達成 |
| R-18表示の実態化・サイト一覧の分類 | 達成 |
| 詳細画面の表示整理(あらすじ/操作/目次の見出し整備) | 達成 |
| DB構造の調査(階層・別DB) | 達成(別項 DB_ARCHITECTURE.md) |

## 追加修正その16 — Swiftエラー一掃・画像の取得対応・バー自動出没・詳細項目

| 指摘 | 対応 |
|---|---|
| Swiftエラーが多すぎる | **実API突合で全修正**: `section.bodyXhtml`(body誤り)/`novel.tocUrl`(sourceUrl誤り)/タプルKeyPath禁止(インデックス列挙へ)/onChange 1引数をiOS17形へ/persistentSystemOverlays引数修正。実フィールド(CoreModels)と全呼び出しを突合済み |
| 表示の絵が未対応(追加時に取得) | **C:取得時に画像を保存** — 本文/前書き/後書きの `<img>` を抽出(既定ルール、`image_pattern` でサイトごとに上書き可=YML対応)し `novels/<域>/<作品>/img/話_番号.拡張子` に保存、本文の src を保存先へ書き換え。画像失敗は本文失敗にしない。テスト追加 **124/124** |
| 画像?挿絵?対応しているか? | はい:リーダーは `<img>` を実画像表示(取得済みローカル画像 or 通信、キャッシュ+プレースホルダ、file パス対応) |
...
## 追加修正その18 — 英日のメリハリ・詳細META・検索サービス発掘・詳細画面の塩梅

| 指摘 | 対応 |
|---|---|
| 英語は英語ではっきり(添えではなく) | **見出し=英語主役(LIBRARY/DISCOVER/FETCH/SETTINGS/STORY/ACTIONS/INDEX/DISPLAY等)・操作ラベル=日本語**のゲーム風メリハリに再設計 |
| 詳細の表示項目 | **META札**を新設: 作者・状態(完結/連載中)・合計話数・コメント数・更新日・更新予定・取得済み(控えめ) |
| 追加時に表示情報を取得 | あらすじ+状態/更新日/更新予定/コメントを**目次取得時に保存**(novel_info_selectors の status/updated/next_update/comment_count 拡張・保存列追加)。サイト側が無い項目は非表示 |
| 詳細画面の塩梅 | **読む=主役**、取得/範囲/書き出しは控えめな1行(大きく出しすぎない)。取得進捗も小サイズに |
| 小説検索の改善+サービス発掘 | **徹底調査**の結果を実装: 「SEARCH ENGINES」欄に **Web小説アンテナ(20サイト横断/検索語連動)**・**ノベレコ**・**なろう検索**・**ランタン検索(R-18レーベル)** を検索専用サービスとして搭載 |
| YAML 対応 | 編集フォームに「詳細表示の抽出」4項目(状態/更新日/更新予定/コメント)を追加 |

## 追加修正その19 — Photos修正・文字比率・検索の本格改良と実地テスト

| 指摘 | 対応 |
|---|---|
| PhotosPicker が見つからない ×3 | `#if canImport(PhotosUI)` で保護し、**PhotosUI が無い環境では UIKit のピッカーに自動フォールバック**。写真ライブラリ使用目的も追記 |
| 大きい文字の多用は風情がない | **比率でメリハリ**へ転換: 画面見出し21–22/セクション15–18/本文13–14/注記10–11 の静かなスケール。字間(tracking)と細い罫で品格を出す |
| 検索の改良 | クエリで **`site:サイト名` による検索範囲指定**に対応。UIにも**範囲チップ(すべて/各サイト)**を追加。サイト単独・横断どちらも可 |
| 検索のテスト | CLI に HTTP トランスポート接続(`http_pipe`)を導入し**実サイトで検証**: なろう/カクヨム/ハーメルン/ノベルアップ+/暁/Arcadia **全6サイトPASS**(各サイト固有のヒットを確認)+ 横断も PASS |
| 検索サービスをもっと | 発掘を継続し**計10サービス**に拡充: Web小説アンテナ/ノベレコ/なろう検索/**ランタン検索(R-18・URL修正)**/**小説を探そうα**/**なろうファンDB**/**スコッパーになろう**/**読み漁り快適化計画**/**あらすじ街灯**/**たぐらん!** |

## 追加修正その20 — R-18 ダウンロード・多ページ目次・リーダー不具合・PhotosUI 撤去

| 指摘 | 根因 | 対応 |
|---|---|---|
| なろう R-18 が DL できない | R-18 作品は novel18 専用ホストにしか無く、ncode.syosetu.com のURLでは JS チャレンジ/404 | **族ホストフォールバック**(ncode↔novel18↔noc↔mid↔mnlt)を実装 — 同一パスを順に試し、成功側へ自動解決。検証: ncode URL の R-18 作品が novel18 へ解決し目次16話を取得 |
| 多ページ目次で最新ページしか取れない | ページ収集パターンのエスケープ事故(`\\?`化)+ p=10〜19 の漏れ+「次へ」依存 | パターンを文字クラスで安全化・全ページ番号を収集・汎用(番号/次へ)と合流・話の重複排除。検証: なろう 2592話/カクヨム 714話/ハーメルン 1536話 全ページ完備 |
| 検索中にサービス一覧が邪魔 | 常時表示 | 検索結果表示中は一覧を隠す |
| メニューが表示できない | 話読み込みの 2.5 秒「自動を隠すタイマー」が手動表示と競合 | タイマー撤去。話移動時のみ一度表示、出没は手動のみ |
| タッチに反応しすぎる | 送りタップ域が広すぎ(28%) | 送りは端の 2 割のみ。中央 6 割 = メニュー出没 |
| 本文が表示されない | UITextView が1画面ぶんしかレイアウトせず残りが切り捨てられていた | textContainer を縦無限でレイアウト(ページング前提)に修正 |
| PhotosUI を使うな | — | **PhotosUI/PhotosPicker を全撤去**。UIKit 標準のUIImagePickerControllerのみで表紙変更 |

## 追加修正その21 — 目次取得の異常耐性・目次UI自動展開・リーダー塩梅

| 指摘 | 根因 | 対応 |
|---|---|---|
| 100話しか取得できない/最新の目次ページ1つしか | 目次ページ walk が1ページでも失敗すると**例外で全体が中断**、済分だけ保存されて打ち切り状態に | **1ページ失敗で止めない**(再キュー1回+スキップ継続)、**番号連鎖プローブ**(?p=N+1 を機械的に追う=リンク解析ゼロでも完走)、範囲外ページ(新規話0)で連鎖停止 |
| 詳細画面で100話しか見れない | 目次リストが手動「さらに表示」ボタン式(初期60話) | **スクロールで自動追加読み込み**(初期200→+300/回)。ボタン操作不要に |
| リーダー/メニューの塩梅 | メニュー見出し大・グリップ無し・タッチ寸法狭い | 「MENU」+小見出しの階調、シートグリップ追加、下部ボタン 54×44 に拡大 |

検証: なろう長編 2593話(連載更新分込み)全ページ完走・R-18族フォールバック 16話 PASS・tests 125/125

## 追加修正その22 — リーダーを「画面に収まる」ページモデルへ

原因: 1ページ=画面高で contentOffset を送っていたため、ページ境界の行が画面端(ノッチ/ホームバー込みのフルブリード領域)で切れていた。加えて textContainer の幅 0 指定が折り返し破壊(横はみ出し)の温床だった。

対応: **電子書籍式の1ページ構成「上余白 + 本文帯 + 下余白」**へ移行 — 余白は contentInset で全ページ均等に確保(セーフエリア分を自動加算)、ページ送りは本文帯単位。幅はビュー追従を明示(widthTracksTextView + 実 bounds 幅)。どのページでも本文は画面内に収まり、端で行が切れない。

## 追加修正その23 — 更新の反映と検索の維持

| 指摘 | 根因 | 対応 |
|---|---|---|
| 小説の更新がされない | ①詳細メタ(更新日/状態/コメント/あらすじ)が追加時の一度きりで再取得されない ②「新規/更新」カウンタが二重計上で数字が不自然 ③目次ページの採番順が実行依存で更新のたびにズレる余地 | ①更新取得のたびにメタ再取得(失敗時は保存値維持) ②新規と更新(改稿再取得)を分離計上 ③ページ処理順をページ番号昇順に固定 |
| 検索が維持されない | タブ切替(`switch tab` + `.id(tab)`)で DiscoverView が破棄され、検索語・結果・範囲が消えていた | 検索状態をシェル所有の `DiscoverSession` に移し、タブを跨いで保持 |
