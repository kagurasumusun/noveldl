...
## 追加修正その8 — リーダーの作り直し(UIKit 標準に一本化)と装丁の刷新

**方針:壊れ続けていた独自描画/ジェスチャを捨て、UIKit 標準部品で「必ず読める」ことを最優先に再構築。**

| 症状 | 根因と対策 |
|---|---|
| 正常に読めない/表示・めくり・開きがまともでない | CoreText 直描画 + 独自ページング + UIView ジェスチャの多層が全ての事故源 → **UITextView ネイティブスクロール**に全面差し替え(描画変換ゼロ)。画像の同期取得(メインスレッド最大20秒ブロック=かくつき)も排除 |
| 動作がかくかく | ①画像の同期取得 ②ページ送りごとの位置情報 JSON 書き込み ③フレームごとの再組版 → すべて除去。書体等の変更は描画のみ差し替え(即時・軽量) |
| メニューがない | **上下バーを常時表示**(閉じる / タイトル / 目次 / Aa・前話/位置/次話)。画面中央タップでもメニュー(文字とレイアウト=テーマ/書体/サイズ/行間/余白/前後の話)が開く |
| カスタマイズができない | テーマ・書体・文字サイズ・行間・余白を **変更即反映**(再取得なし)。設定アプリ側と共通の保存 |
| 送りたいのに戻る/勝手に次話 | タップゾーン:左=1画面めくり戻し(先頭では停止)/ 右=1画面めくり送り(末尾でのみ次話)/ 中央=メニュー。バーの前話/次話ボタンでも確実に移動 |
| 開けない話がある | 「この話を取得」ボタンでその場で 1 該取得して開く(取得済みでなくても読める) |
| 小説詳細が安っぽい | 書影の**布を帯に展開した装丁**(布帯 + 双罫 + 箔押しの書名)+ 書影が帯に掛かる構成に刷新 |


## 追加修正その9 — ダーク化・メニュー自動非表示・再取得防止・軽量化・設定の場所

| 指摘 | 対応 |
|---|---|
| 色が気に入らない → ダーク系(暗すぎず明るすぎず) | アプリ全体を温かいダーク(canvas #1B1A18 / surface #252320 / 文字 #ECE8E0)に統一。行動色の煉瓦はダーク映えに調整。読書テーマ(紙/セピア/夜)は本文用に選択肢のまま |
| メニュー常時表示は微妙 → 隠れてもよくない? | **3.2 秒で自動非表示**。中央タップで再表示(操作中は隠れない)。送り操作時はバーを退場。リキッドグラス等の演出は入れず、フラットのまま |
| 機能の置き場所(設定だから設定にまとめるのではなく) | **読書設定はリーダーのメニューへ移管**(テーマ/書体/サイズ/行間/余白 + 送り方)。設定アプリ側は取得・サイト・クッキーのみに整理 |
| 横スクロール・紙捲りなどリーダー設定 | 「左右スワイプで送り」を追加(ON/OFF、タップ送りと共存)。話の切り替えは静かなフェード。効果過多の紙捲りは入れない(フラット方針) |
| ダウンロード済みをまたダウンロード | C:プレースホルダ更新時に取得済みフラグを**無条件で保持**するよう修正(取りこぼし防止)。取得済み話は署名一致でスキップされ、**改稿(署名変化)話のみ再取得**=改稿対応。結果は「新規 x・更新 y・スキップ z」と必ずカウント表示 |
| とにかく重い | 書影の影を全廃(オフスクリーン描画の削減)、押下アニメを軽量化、書影内の GeometryReader を塗り潰し化、設定のプリセット再読込を停止、読書中は章リストをキャッシュ |

## 追加修正その10 — 読書メニューの分離・めくり演出・アニメ度合いの調整

| 指摘 | 対応 |
|---|---|
| メニューと読書時のメニューは別であるべき | **「読書メニュー」を新設**(読書専用シート:表示/送り/移動の3区分)。中央タップ or Aa で開く。アプリ設定(取得・サイト・クッキー)とは完全分離 |
| 機能の置き場所の見直し | 読書の操作=読書メニュー(テーマ/書体/サイズ/行間/余白/めくり/スワイプ/目次/前後の話)、作品の操作=作品詳細、アプリの操作=設定、に再整理 |
| ページめくりアニメーション | **紙捲り(カール)/スライド/フェード/なし**を選択可(既定=紙捲り)。話の移動は静かなフェード + 成功ハプティクス |
| はきはきしすぎ → 度合い | タイミングを再調整:バーの表示 0.35s・退場 0.45s(easeInOut)、押下 0.2s、セグメント 0.22s、めくり 0.26–0.36s。急がないが遅くもない |
| 細かな工夫 | ①初回だけ操作ヒント(中央タップ/左右送り)を表示して消える ②目次に「改」マーク表示 ③話移動で成功ハプティクス ④取得中でも読書メニューの即時反映は維持 |

## 追加修正その11 — 読書メニュー拡張・ページング化・棚レイアウト・設定の簡素化・DB調査

| 指摘 | 対応 |
|---|---|
| 常に戻るボタンは不要 | トップバーの常時「戻る」を廃止。「閉じて作品詳細へ」は読書メニューの中へ |
| 表示は2つでなく横一つ+自由に | 本棚のレイアウトを **横並び1列(既定)/グリッド/リスト** から選択可(ヘッダーの切替ボタン) |
| 数百〜数千話の詳細が重い | 目次を**段階表示**(初期60話+「さらに200話」)に。章配列はロード時に1回だけキャッシュ |
| リーダーの下移動をページに | 連続スクロールを廃止し **1スクリーン=1ページ**(`isPagingEnabled`)。送りもページ境界に揃え |
| パーサー管理・クッキーエディタ・コマンドはいらない | **削除**。「ユーザーに操作させるものではない」= パーサ/クッキー/取得コマンドはアプリ内部の仕事。設定は「取得の間隔・対応サイト(閲覧のみ)・情報」に |
| ルビ/ヘッダー/フッター等の設定 | 読書メニューの「表示項目」に **ルビ(振り仮名)/ヘッダー(タイトル)/フッター(進捗)** を追加。行間・書体・サイズ・余白は既存 |
| 小説DBの構造(分け/階層/別DB?) | `DB_ARCHITECTURE.md` に調査結果を記載。結論:カタログ1本+作品ごとSQLiteシャード+FTS5 = SQLite公式の sharding 方針と database-per-tenant パターンに合致した適切な構造 |
